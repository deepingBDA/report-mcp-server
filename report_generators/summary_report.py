...
class SummaryReportGenerator(BaseWorkflow[SummaryReportState]):
    def __init__(self) -> None:
        ...
        # 1일 모드용 프롬프트 (DA 요구사항 Few-shot 방식)
        ...
        self._action_prompt_tpl = (
            """
            1일(일자별) 데이터를 바탕으로 정확히 아래 예시 형식을 따라 매장별 액션을 HTML 형식으로 작성하세요.

            [출력 예시]
            
            예시 입력 데이터:
            타워팰리스점: 1,840명, +4.0%
            신촌르메이에르점: 1,129명, +17.1%
            역삼점: 1,166명, +1.4%
            만촌힐스테이트점: 1,575명, -0.2%
            금천프라임점: 577명, +11.4%
            마천파크점: 687명, +5.5%
            ...
            올바른 출력:
            <li><strong>타워팰리스점</strong>: 증가폭이 둔화되고 있어 성장세 유지 전략 검토 필요</li>
            <li><strong>신촌르메이에르점</strong>: 지속적 증가세로 핵심 상품 파악 효과 확인 및 성공 요인 확산 필요</li>
            <li><strong>역삼점</strong>: 방문 규모는 크지만 증감률이 낮아 충성 고객 유지 전략 요구</li>
            <li><strong>만촌힐스테이트점</strong>: 방문 규모는 크지만 증감률이 낮아 충성 고객 유지 전략 요구</li>
            <li><strong>금천프라임점</strong>: 방문객 수 저조로 지역 맞춤형 마케팅 및 매장 이벤트 강화 필요</li>
            <li><strong>마천파크점</strong>: 방문객 수 저조로 지역 맞춤형 마케팅 및 매장 이벤트 강화 필요</li>

            [핵심 규칙]
            1. 각 매장별로 <li><strong>매장명</strong>: 구체적 현황 분석 및 실무적 액션</li> 형식 사용
            2. 매장 상황별 액션 패턴:
               - 고성장 매장: "지속적 증가세로 핵심 상품/요인 파악 및 확산 필요"
               - 증가폭 둔화: "증가폭이 둔화되고 있어 성장세 유지 전략 검토 필요"  
               - 방문객 많지만 증감률 낮음: "방문 규모는 크지만 증감률이 낮아 충성 고객 유지 전략 요구"
               - 방문객 수 저조: "방문객 수 저조로 지역 맞춤형 마케팅 및 매장 이벤트 강화 필요"
            3. 실무진이 바로 실행할 수 있는 구체적 액션 제시
            4. 증감률(+/-%)은 표시하지 않고 현황 해석과 액션에 집중
