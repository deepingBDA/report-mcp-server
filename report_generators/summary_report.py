...
class SummaryReportGenerator(BaseWorkflow[SummaryReportState]):
    def __init__(self) -> None:
        ...
        self._action_prompt_tpl = (
            """
            1일(일자별) 데이터를 바탕으로 증감률이 절댓값 10%를 초과하는 매장들만 선별하여 정확히 아래 예시 형식을 따라 매장별 액션을 HTML 형식으로 작성하세요.

            **중요: 증감률이 +10% 초과 또는 -10% 미만인 매장만 액션을 작성하고, 그 외 매장(±10% 포함)은 제외하세요.**

            [출력 예시]
            
            예시 입력 데이터:
            신촌르메이에르점: 1,129명, +17.1%  ← 10% 초과 증가
            금천프라임점: 577명, +11.4%      ← 10% 초과 증가  
            만촌점: 800명, -12.5%              ← 10% 미만 감소
            일반점: 900명, +10.0%              ← 정확히 10%는 제외
            
            올바른 출력 (절댓값 10% 초과 매장만):
            ...
            <li><strong>만촌점</strong>: 급격한 감소세로 긴급 원인 분석 및 개선 전략 필요</li>

            [핵심 규칙]
            1. **필수**: 증감률이 절댓값 10%를 초과하는 매장만 선별하여 액션 작성
            2. 각 매장별로 <li><strong>매장명</strong>: 구체적 현황 분석 및 실무적 액션</li> 형식 사용
            3. 매장 상황별 액션 가이드:
               - +10% 초과 증가: 핵심 성공 요인 파악 및 확산 방안 검토
               - -10% 미만 감소: 긴급 원인 분석 및 개선 전략 수립
            4. 절댓값 10% 이하 매장(±10% 포함)은 제외하고 출력하지 않음
            5. 실무진이 바로 실행할 수 있는 구체적 액션 제시
