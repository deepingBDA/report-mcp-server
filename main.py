...
async def send_html_email_report(request: EmailReportRequest):
    ...
    try:
        ...
        if not smtp_user or not smtp_password:
            raise HTTPException(status_code=400, detail="SMTP 사용자명 또는 패스워드가 설정되지 않았습니다")
        
        # 받는 사람 설정
        to_emails = request.to_emails or os.getenv("EMAIL_TO", "").split(",")
        cc_emails = request.cc_emails or (os.getenv("EMAIL_CC", "").split(",") if os.getenv("EMAIL_CC") else [])
        to_emails = [email.strip() for email in to_emails if email.strip()]
        cc_emails = [email.strip() for email in cc_emails if email.strip()]
        
        if not to_emails:
            raise HTTPException(status_code=400, detail="받는 사람 이메일이 설정되지 않았습니다")
        
        # 제목에 접두사 추가
        subject_prefix = os.getenv("EMAIL_SUBJECT_PREFIX", "[일일 리포트]")
        full_subject = f"{subject_prefix} {request.subject}"
        
        # HTML 파일 읽기
        if not os.path.exists(request.html_path):
            raise HTTPException(status_code=404, detail=f"HTML 파일을 찾을 수 없습니다: {request.html_path}")
        
        with open(request.html_path, 'r', encoding='utf-8') as f:
            html_content = f.read()
        ...
        result = f"✅ **이메일 발송 완료!**\\n\\n**제목:** {full_subject}\\n**받는 사람:** {', '.join(to_emails)}\\n**참조:** {', '.join(cc_emails) if cc_emails else '없음'}\\n**발송자:** {smtp_user}"
...
async def get_email_config():
    ...
    try:
        ...
        config = {
            ...
            "default_recipients": {
                "to": email_to.split(",") if email_to else [],
                "cc": email_cc.split(",") if email_cc else []
            },
            "subject_prefix": subject_prefix
        }
        
        result = f"📧 **이메일 설정:**\\n\\n**SMTP 서버:** {config['smtp_server']}\\n**사용자:** {config['smtp_user']}\\n**패스워드:** {'✅ 설정됨' if config['smtp_password_set'] else '❌ 미설정'}\\n**받는 사람:** {', '.join(config['default_recipients']['to']) if config['default_recipients']['to'] else '미설정'}\\n**참조:** {', '.join(config['default_recipients']['cc']) if config['default_recipients']['cc'] else '없음'}\\n**제목 접두사:** {config['subject_prefix']}"
        
        return {"result": result, "config": config}
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"이메일 설정 조회 실패: {e}")
...
async def test_email_connection():
    ...
    try:
        ...
        if not smtp_user or not smtp_password:
            return {"result": "❌ SMTP 사용자명 또는 패스워드가 설정되지 않았습니다. .env 파일을 확인하세요."}
        ...
        try:
            ...
            result = f"❌ **SMTP 연결 실패**\\n\\n**서버:** {smtp_host}:{smtp_port}\\n**오류:** 서버에 연결할 수 없습니다 ({e})"
            return {"result": result, "status": "connection_failed"}
            
    except Exception as e:
        logger.error(f"SMTP 연결 테스트 오류: {e}")
        raise HTTPException(status_code=500, detail=f"SMTP 연결 테스트 실패: {e}")
